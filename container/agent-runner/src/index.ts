...
// Path overrides for local mode (set by container-runner when AGENT_LOCAL_MODE=1).
// In container mode these env vars are unset, so defaults to /workspace/*.
const GROUP_DIR = process.env.NANOCLAW_WORKSPACE_GROUP || '/workspace/group';
const GLOBAL_DIR = process.env.NANOCLAW_WORKSPACE_GLOBAL || '/workspace/global';
